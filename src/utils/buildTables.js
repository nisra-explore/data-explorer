    ...
    if (other_vars.length > 0 || !isDP) {
        ...
        for (let i = 0; i < table_count; i++) {
            ...
            if (!isDP && breakdown_dim) {
                ...
                const selected_breakdown_code = document.getElementById(other_vars[0]).value;

                const breakdown_index = breakdown_dim.categories.findIndex(c => c.code === selected_breakdown_code);
                ...
                for (let geography = 0; geography < geography_count; geography++) {
                    ...
                    const value_index = (geography * statistic_count) + statistic_index;

                    headline_total += values[value_index] || 0;
                }
            }
            ...
            if (isDP) {
                ...
                let td_0 = document.createElement("td");
                td_0.textContent = statistic_dim.categories[statistic_index].label;

                tr.appendChild(td_0);

                let td_1 = document.createElement("td");
                td_1.textContent = headline_total.toLocaleString("en-GB");
                td_1.style = "text-align: right;";
